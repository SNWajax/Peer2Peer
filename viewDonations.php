...
<html onload = "loadMarkers()">
	...
	function addMarker(aItem,aqty,anEmail,aItemID,alat,alng){
		...
		marker.addListener('click', function() {
			if (confirm('Are you sure you want to recieve ' + aItem + '?')) {
				if(aqty > 1){
					ans = parseInt(prompt("How many "+aItem+ " would you like?"))
					if(ans <= 0 || ans > aqty)
						return;
				}
				else	
				...
				url = "back/alertDonationsUsers.php";
				request.open("POST", url, true);
				request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				request.send("donnerEmail="+anEmail+"&qty="+aqty+"&item="+aItem+"&aItemID="+aItemID+"&requestAmount="+ans);
			} else {
				console.log(anEmail);
			}
		});
